#!/usr/bin/env python3
"""
Script to send volume commands to an Android TV device using MCP.
"""
...
async def main():
    """Main function to send a volume command."""
    common_parser = create_common_parser()
    parser = argparse.ArgumentParser(
        description="Send a volume command to an Android TV device.",
        parents=[common_parser],
    )
    parser.add_argument(
        "action",
        choices=["up", "down", "mute"],
        help="The volume action to perform.",
    )
    ...
    async with MCPClient(server_params) as client:
        ...
        await client.send_mcp_command("atv_volume", arguments)
